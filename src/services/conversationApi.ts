...
class ConversationApiService {
  ...
  async findDetailById(conversationId: string): Promise<ConversationView> {
    const response = await this.api.get<ConversationView>(`/${conversationId}`);
    return response.data;
  }
  ...
  async clearConversationHistory(conversationId: string) {
    const response = await this.api.post(`/${conversationId}/clear-history`);
    return response.data;
  }
  ...
  async uploadSingle(
    ...
    if (options?.messageType) {
      formData.append("messageType", String(options.messageType).toUpperCase());
    }
    ...
    return {
      ...
      fileExtension: String(data.fileExtension || fallbackMeta.fileExtension),
  ...
  async uploadBatch(
    ...
    const response = await this.postMessageFormData("/upload-batch", formData);
    if (!response.ok) {
        ...
        ? (data.items as UploadBatchResponseItem[]).map(
            (item, index: number) => {
              ...
              return {
                ...
                messageType: String(item?.messageType || fallback.messageType),
                fileExtension: String(
                  item?.fileExtension || fallback.fileExtension,
                ),
                fileCategory: item?.fileCategory || fallback.fileCategory,
  ...
  async sendSingleFile(payload: {
    ...
    if (payload.messageType) {
      formData.append("messageType", String(payload.messageType).toUpperCase());
    }
  ...
  async sendBatchFiles(payload: {
    ...
    const response = await this.postMessageFormData("/send-files", formData);
    if (!response.ok && response.status === 404) {
    ...
    const data = await response.json();
    return {
      conversationId: String(data.conversationId || payload.conversationId),

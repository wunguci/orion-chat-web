...
export const ChatPage: React.FC = () => {
      ...
      setGroupInfoOverrides((prev) => {
        ...
        return {
          ...
          [payload.groupId]: {
            ...
            ...(payload.groupAvatar
              ? {
                  groupAvatar: toAbsoluteMediaUrl(payload.groupAvatar),
                }
              : {}),
          },
        };
  ...
  const getMessageKey = useCallback(
    (message: Pick<SocketMessage, "id" | "clientMessageId">) => {
      const hasPersistedId = !!message.id && /^[a-f0-9]{24}$/i.test(message.id);
  ...
  useEffect(() => {
    ...
    notificationSocket.on("notifications:updated", handleNotificationUpdated);
  ...
  const toSocketMessage = useCallback(
    (payload: IncomingSocketPayload): ChatSocketMessage => {
      ...
      const resolvedMimeType =
        ...
        attachment?.fileType;
      const messageType =
        payload?.messageType || payload?.type || attachment?.messageType;
      ...
      const normalizedReplyPreview = payload?.replyToMessagePreview
        ? {
            ...
            senderName:
              payload.replyToMessagePreview.senderName || "Tin nhắn gốc",
            content:
            ...
            snippet:
              ...
              payload.replyToMessagePreview.content ||
              "",
            createdAt: payload.replyToMessagePreview.createdAt || undefined,
          }
        : null;
          ...
          if (matched && matchedMediaUrl) {
            ...
            const hydrated = toSocketMessage({
              ...
              senderName:
                ...
                matched.senderId,
              senderAvatar:
                matchedWithSender.senderAvatar || message.senderAvatar,
      ...
      setSocketMessages((prev) => {
        ...
        if (existingIndex >= 0) {
          ...
          next[existingIndex] = {
            ...
            replyToMessageId: msg.replyToMessageId || current.replyToMessageId,
            replyToMessagePreview:
              msg.replyToMessagePreview ||
              current.replyToMessagePreview ||
              null,
            messageStatus: msg.messageStatus || current.messageStatus,
          };
          return next;
        }
      ...
      try {
        ...
        members.forEach((member) => {
          if (member.userId !== USER_ID) {
            participantNames[member.userId] = member.fullName || "Member";
          }
        });
        ...
        {selectedConversation ? (
          <>
            ...
            <ChatHeader
              ...
              isBlocked={isPrivateConversation && (iAmBlocked || iAmTheBlocker)}

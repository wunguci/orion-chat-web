...
export const MessageList: React.FC<{
    ...
    .filter(
      (m): m is SocketMessage & { fileUrl: string; fileType: string } =>
        ...
        (m.fileType?.startsWith("image/") === true ||
          ...
          m.type === "image" ||
          /\.(jpg|jpeg|png|gif|webp|bmp|svg)$/i.test(m.fileName || "")),
    )
    .map((m) => ({
      src:
  ...
  // Gom các tin nhắn ảnh liên tiếp của cùng người gửi thành 1 group
  type MessageItem =
  ...
  const isImageMessage = (m: SocketMessage) =>
    !m.isRecalled &&
    m.isFile &&
    !!m.fileUrl &&
    (m.fileType?.startsWith("image/") === true ||
  ...
  const groupedItems = useMemo((): MessageItem[] => {
    ...
    let i = 0;
    while (i < socketMessages.length) {
      const msg = socketMessages[i];
      if (isImageMessage(msg)) {
        // Collect consecutive image messages from same sender
        const group: SocketMessage[] = [msg];
        let j = i + 1;
        while (
          j < socketMessages.length &&
          isImageMessage(socketMessages[j]) &&
          socketMessages[j].senderId === msg.senderId
        ) {
          group.push(socketMessages[j]);
          j++;
        }
        if (group.length === 1) {
  ...
  const imageIndexMap = useMemo(() => {
    ...
    for (const m of socketMessages) {
      if (
        ...
        map.set(m.clientMessageId || m.id, counter);
        counter++;
      }
    }
    return map;
  ...
  const renderImageGrid = (msgs: SocketMessage[], isMe: boolean) => {
    ...
    // Layout: 1→full, 2→2col, 3→1 large + 2 small, 4→2x2, 5+→3col grid với overlay "+N"
    const visibleMsgs = count > 5 ? msgs.slice(0, 5) : msgs;
    const hiddenCount = count > 5 ? count - 5 : 0;
  ...
  const getSeenCount = (message: SocketMessage) => {
    ...
    return message.seenBy.reduce((count, item) => {
      ...
      return item.userId && item.userId !== currentUserId ? count + 1 : count;
    }, 0);
  };
          ...
          groupedItems.map((item) => {
            ...
            const hasImage =
              ...
              (msg.fileType?.startsWith("image/") === true ||
                ...
                msg.type === "image" ||
                /\.(jpg|jpeg|png|gif|webp|bmp|svg)$/i.test(msg.fileName || ""));
            const imgIdx =
              imageIndexMap.get(msg.clientMessageId || msg.id) ?? 0;
            const isVideoFile =
              msg.isFile &&
                      ...
                      {getReplyPreview(msg) && (
                        ...
                        >
                          <div className="font-semibold">
                            {getReplyPreview(msg)?.senderName || "Tin nhắn gốc"}
                          </div>
                          <div
                      ...
                      >
                        {msg.isPinned && (
                          <div
                            className={`mb-1 text-[11px] font-semibold ${
                              isMe ? "text-green-100" : "text-amber-600"
                            }`}
                          >
                        ...
                        >
                          ...
                          {isMe && (
                            ...
                            >
                              {getUploadStatusLabel(msg.uploadStatus)}
                            </span>
                          )}
                          {isMe && getSeenCount(msg) > 0 && (
                            <span className="ml-2 text-emerald-200 font-medium">
                              Đã xem {getSeenCount(msg)}
                            </span>
                          )}
                        </p>
                        {isMe &&
                    ...
                    >
                      ...
                      {openActionMenuKey === messageKey && (
                        ...
                        >
                          ...
                          <button
                            onClick={() => {
                              onTogglePinMessage?.(msg, !msg.isPinned);
                              setOpenActionMenuKey(null);
                            }}
                            className="w-full px-4 py-2 text-left text-sm text-slate-700 hover:bg-slate-100 border-b border-slate-100"
                          >
